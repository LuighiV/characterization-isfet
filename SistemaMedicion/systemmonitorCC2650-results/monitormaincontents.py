...
class MonitorMainContents(mmw.MeasureMainW):
    ...
    def initGUIContents(self):
        ...
        ##Group view
        #######################################
        self.livephplots = lvcplt.LiveTwoPlotting()
        self.livetempplot = lvcplt.LiveOnePlotting()
        self.measureph= ms.MonitorTwoView(self.livephplots,['pH compensated','pH no compensated'])
        self.measuretemp = ms.MonitorOneView(self.livetempplot,'NTC')
        
        self.livetempplot.setMinimumWidth(500)
        
        lview = QtGui.QGridLayout()
        lview.addWidget(self.livephplots,0,0)
        lview.addWidget(self.measureph,0,1)
        lview.addWidget(self.livetempplot,1,0)
        lview.addWidget(self.measuretemp,1,1)
