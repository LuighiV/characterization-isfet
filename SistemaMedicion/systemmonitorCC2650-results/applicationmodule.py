...
class Application(QtGui.QApplication):
    ...
    def startprocess(self):
        serial= self.serial
        livephplots= self.gui.livephplots
        livetempplot= self.gui.livetempplot
        measureph= self.gui.measureph
        measuretemp = self.gui.measuretemp
        ...
        d2 = threading.Thread(name="threadtempread",
                                  ...
                                  args=(self.start,livephplots,livetempplot,recording))
        d2.start()
        ...
        self.t.timeout.connect(livephplots.updateLiveData)
        self.t.timeout.connect(livetempplot.updateLiveData)
        self.t.timeout.connect(measureph.updateData)
        self.t.timeout.connect(measuretemp.updateData)
        self.t.start(25)
