...
class RecordingWidget(rec.RecordingWidget):
    ...
    def recordTempData(self,tempntc,volt,ph,glass):
        self.f.write(', '.join([str(tempntc),str(volt),str(ph),str(glass)]) + '\n')
    
    def recordpHData(self,phcomp,ph,temp):
        self.f.write(', '.join([str(phcomp),str(ph),str(temp)]) + '\n')